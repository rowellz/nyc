import {treePitYaw as $treePitYaw} from './fixtures.js';
import {j as $TreeCylinder,Gt as $TreePlane,h as $TreeAttribute} from './textureRelease-2U-gT89r.js';
...
import { syncTunnelTerrain as $tunnelTerrain } from './tunnels.js';
...
vec3 nonPerturbedNormal = normal;`).replace(`#include <map_fragment>`,`#include <map_fragment>
          ...
          totalEmissiveRadiance += diffuseColor.rgb * vec3(1.5, 1.34, 0.42) * (0.06 + ${t?`0.24`:`0.85`} * treeBack * treeBack * treeBack) * treeUnder * treeThin * clamp(uTreeSun.y * 3.0, 0.0, 1.0);`)}),n},g=zt(),v=Vt();g&&f.push(g),v&&f.push(v);for(let e of Ot){let t=Lt(e),r=kt(e),i=n.bark[At[e].bark];const $lod=$treeLods(t,At[e],{CylinderGeometry:$TreeCylinder,PlaneGeometry:$TreePlane,BufferAttribute:$TreeAttribute},Mt);u.set(e,{wood:p(t.wood,new _({map:i.map,normalMap:i.normal,roughnessMap:i.rough??null,roughness:.95}),`env-tree-${e}-wood`,!0),leaves:p(t.leaves,h(n.leaves[r],!1),`env-tree-${e}-leaves`,!0),middle:p($lod.middle,h(n.leaves[r],!1),`env-tree-${e}-middle`),farWood:p($lod.wood,new _({map:i.map,roughness:.95}),`env-tree-${e}-far-wood`),far:p($lod.far,h(n.crowns[r],!0),`env-tree-${e}-far`),seeds:t.seeds?p(t.seeds,new _({map:g??void 0,color:g?16777215:5917228,alphaTest:.5,side:2,roughness:.9}),`env-tree-${e}-seeds`):null})}let y=p(new l(Wt,Gt).rotateX(-Math.PI/2),new _({map:n.soil.map,normalMap:n.soil.normal,roughness:1,polygonOffset:!0,polygonOffsetFactor:-2,polygonOffsetUnits:-2}),`env-tree-pits`),b=v?p(new l(1,1).rotateX(-Math.PI/2),new _({map:v,transparent:!0,opacity:.8,alphaTest:.12,depthWrite:!1,roughness:.9,polygonOffset:!0,polygonOffsetFactor:-3,polygonOffsetUnits:-3}),`env-tree-litter`):null,x=[],S=Wt/2,w=Gt/2;for(let e of[-1,1]){for(let t of[.16,.46])x.push(new I(Wt,.028,.028).translate(0,t,e*w)),x.push(new I(.028,.028,Gt).translate(e*S,t,0));for(let t of[-1,1])x.push(new I(.04,.52,.04).translate(e*S,.26,t*w));for(let t=-3;t<=3;t++)x.push(new I(.014,.3,.014).translate(t*.32,.31,e*w));for(let t=-2;t<=2;t++)x.push(new I(.014,.3,.014).translate(e*S,.31,t*.3))}let T=p(Mt(x),new _({color:1316372,roughness:.55,metalness:.75}),`env-tree-guards`),E=new a,D=new B(1,1,1),O=!0,k=-1/0,A=new m(1/0,1/0,1/0),j=e.quality.level===`low`||e.quality.level===`mobile`?65:e.quality.level===`medium`?90:120,M=Math.min(j,70),N=$treeBudget.distance,P=(e,t)=>{let n=jt(e.species);if(e.species.toLowerCase()!==`tree`||!t)return n;let i=r.uSafe.value;return Math.hypot(e.x-i.x,e.z-i.z)<i.z+90?`allee`:U(e.x,e.z,8)<.5?`plane`:`oak`};return{addTile(e,$scenery=false){let t=[];for(let n of e.trees){if(![n.x,n.z,n.dbh,n.height].every(Number.isFinite))continue;let r=n.park??e.parks.some(e=>H(n.x,n.z,e)),i=P(n,r),a=c.clamp(.75+Math.max(0,n.dbh)*.02,.8,1.35),o=c.clamp(n.height*(.94+U(n.x,n.z,4)*.12),3,32),s=o*a*(.9+U(n.x,n.z,5)*.2);E.position.set(n.x,0,n.z),E.rotation.set(0,U(n.x,n.z)*Math.PI*2,0),E.scale.set(s,o,s*(.92+U(n.x,n.z,6)*.16)),E.updateMatrix();let l=i===`plane`||i===`allee`,u=new B().setHSL(.21+U(n.x,n.z,7)*.05,.25,(l?.62:.68)+U(n.x,n.z,1)*(l?.14:.2));!l&&U(n.x,n.z,2)<.08&&u.setRGB(1.12,.95,.65),t.push({tree:n,form:i,matrix:E.matrix.clone(),tint:u,street:!r,guard:U(n.x,n.z,3)<.5,pitYaw:$scenery||r?0:Ut(e,n.x,n.z),litter:$scenery?[]:Bt(n.x,n.z,s*At[i].width,i),scenery:$scenery})}($scenery?$treeTiles:o).set(e.key,t),O=!0},removeTile(e){o.delete(e),O=!0},update(t){let n=e.camera.position,i=r.uWind.value.length();for(let e of s)e.windBounds(i);if($syncSceneryTrees(e,this,$treeTiles))O=true;if(!O&&(t-k<.15||A.distanceToSquared(n)<.25))return;O=!1,k=t,A.copy(n);for(let e of s)e.count=0;let a=new Set;for(let t of $treeRecords(o,$treeTiles,n,N,$treeBudget.count)){let e=(t.tree.x-n.x)**2+(t.tree.z-n.z)**2;if(e>N*N)continue;let r=u.get(t.form);if(!t.scenery&&e<=j*j){if(r.wood.add(t.matrix,D),r.leaves.add(t.matrix,t.tint),e<=M*M&&(r.seeds?.add(t.matrix,D),b))for(let e of t.litter)a.has(e.key)||(a.add(e.key),E.position.set(e.x,.034,e.z),E.rotation.set(0,e.yaw,0),E.scale.set(e.size,1,e.size),E.updateMatrix(),b.add(E.matrix,D));t.street&&(E.position.set(t.tree.x,Kt,t.tree.z),E.rotation.set(0,t.pitYaw,0),E.scale.set(1,1,1),E.updateMatrix(),y.add(E.matrix,D),t.guard&&T.add(E.matrix,D))}else{r.farWood.add(t.matrix,D);(e<=$treeBudget.middle**2?r.middle:r.far).add(t.matrix,t.tint)}}for(let e of s)e.finish(),e.windBounds(i)},async prepare(e){let t=new C;for(let e of s)t.add(e.sample());try{await e(t)}finally{for(let e of t.children)e.dispose();t.clear()}},setLeafCards(e){for(let t of Ot){let n=u.get(t).leaves.mat;n.map=e[kt(t)],n.needsUpdate=!0;const middle=u.get(t).middle.mat;middle.map=e[kt(t)];middle.needsUpdate=true}},setBark(e,t){for(let n of Ot){if(At[n].bark!==e)continue;let r=u.get(n).wood.mat;r.map=t.map,r.normalMap=t.normal,r.roughnessMap=t.rough,r.needsUpdate=!0;const farWood=u.get(n).farWood.mat;farWood.map=t.map;farWood.needsUpdate=true}},inPit(e,t){let n=Wt/2,r=Gt/2;for(let i=Math.floor((e-n)/256);i<=Math.floor((e+n)/256);i++)for(let a=Math.floor((t-n)/256);a<=Math.floor((t+n)/256);a++)for(let s of o.get(ie(i,a))??[]){if(!s.street)continue;let i=e-s.tree.x,a=t-s.tree.z;if(i*i+a*a>n*n)continue;let o=Math.cos(s.pitYaw),c=Math.sin(s.pitYaw);if(Math.abs(i*o-a*c)<=n&&Math.abs(i*c+a*o)<=r)return!0}return!1},dispose(){o.clear();$treeTiles.clear();for(let e of s)e.dispose();for(let e of f)e.dispose()}}}var $=new s(-.875,-.485),Jt=new s(.485,-.875),Yt=new s(-1506.3,-1068.2),Xt=80.5,Zt=-.06,Qt=.9,$t=.18,en=[{street:36,length:240,width:60,stumps:!0},{street:39,length:150,width:45,shed:6},{street:41,length:190,width:26,stumps:!0},{street:42.7,length:170,width:32},{street:44,length:200,width:60,stumps:!0},{street:46,length:265,width:40,carrier:!0},{street:48,length:330,width:62,shed:13},{street:50,length:330,width:62,shed:13},{street:52,length:330,width:62,shed:13},{street:54,length:200,width:62,shed:12},{street:57,length:150,width:60}];function tn(e){return Yt.clone().addScaledVector(Jt,(e.street-44)*Xt)}var nn=`
