import{n as e}from"./quality-BuEwAkMy.js";import{c as t,d as n,u as r}from"./index-DQv-X5z6.js";var i=`'Barlow Condensed', 'Arial Narrow', 'Helvetica Neue', Arial, sans-serif`,a=`'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif`,o=`nyc-ui-style`,s=`nyc-ui-fonts`;function c(e=!0){if(e&&!document.getElementById(s)){let e=document.createElement(`link`);e.rel=`preconnect`,e.href=`/world/assets/fonts`,e.crossOrigin=`anonymous`,document.head.appendChild(e);let t=document.createElement(`link`);t.id=s,t.rel=`stylesheet`,t.href=`/world/assets/fonts/fonts.css`,document.head.appendChild(t)}if(document.getElementById(o))return;let t=document.createElement(`style`);t.id=o,t.textContent=l,document.head.appendChild(t)}var l=`
#nyc {
  --head: ${i};
  --body: ${a};
  --fg: #f4f6f8;
...
`,u=`nyc.name`;function d(e){try{return localStorage.getItem(e)}catch{return null}}var f=[[`WASD`,`move`],[`Shift / Ctrl or Alt`,`sprint / walk · C crouch`],[`Mouse`,`look · LMB fire · RMB aim`],[`E`,`enter / exit vehicle · pick up`],[`F`,`horn / siren`],[`R`,`reload`],[`Tab`,`leaderboard`],[`M`,`map`]];function p(e,t,n){let r=document.createElement(e);return t&&(r.className=t),n?.appendChild(r),r}function m(){return(navigator.maxTouchPoints>0&&matchMedia(`(pointer: coarse)`).matches?[[`Left stick`,`move / drive`],[`Right drag`,`look`],[`Jump`,`jump / handbrake`],[`Interact`,`enter / exit vehicle · pick up`],[`Fire`,`shoot`],[`Reload`,`reload`]]:f).map(([e,t])=>`<span><b>${e}</b>${t}</span>`).join(``)}var h=class{el;visible=!1;constructor(e,t){this.el=p(`div`,`screen ia hidden ${t}`,e)}get isVisible(){return this.visible}open(){this.visible||(this.visible=!0,this.el.classList.remove(`hidden`,`out`))}hide(e=!1){this.visible&&(this.visible=!1,e?(this.el.classList.add(`out`),setTimeout(()=>{this.visible||this.el.classList.add(`hidden`)},360)):this.el.classList.add(`hidden`))}},g=class extends h{onSubmit;input;err;email;newsletter;constructor(e,i){super(e,`intro`),this.onSubmit=i,p(`div`,`title head`,this.el).textContent=`New York`,p(`div`,`sub`,this.el).textContent=`a city that never ends`;let a=p(`form`,void 0,this.el);this.input=p(`input`,void 0,a),this.input.type=`text`,this.input.maxLength=100,this.input.name=`name`,this.input.required=!0,this.input.setAttribute(`aria-label`,`Your name`),this.input.placeholder=`Your name`,this.input.autocomplete=`given-name`,this.input.spellcheck=!1;let o=p(`small`,`name-help`,a);o.id=`public-name-help`,o.textContent=`You'll appear in the city as a random name like amber-fox-42`,this.input.setAttribute(`aria-describedby`,o.id),this.email=p(`input`,void 0,a),this.email.type=`email`,this.email.name=`email`,this.email.required=!0,this.email.maxLength=254,this.email.autocomplete=`email`,this.email.placeholder=`Your email`,this.email.setAttribute(`aria-label`,`Your email`);let s=p(`div`,`newsletter`,a),c=p(`label`,void 0,s);this.newsletter=p(`input`,void 0,c),this.newsletter.type=`checkbox`,this.newsletter.name=`newsletter`,this.newsletter.checked=!0,this.newsletter.setAttribute(`aria-describedby`,`newsletter-help`),p(`span`,void 0,c).textContent=`Also sign up for the Something Big newsletter to get the latest from Matt Shumer`;let l=p(`small`,void 0,s);l.id=`newsletter-help`,l.textContent=`You can play without it.`;let u=p(`button`,`btn`,a);u.type=`submit`,u.textContent=`Enter the city`;let d=p(`p`,`privacy`,this.el);d.append(`Your name and email are not shown to other players. The `);let f=p(`a`,void 0,d);f.href=`https://somethingbig.ai/privacy#world-game`,f.target=`_blank`,f.rel=`noopener noreferrer`,f.textContent=`Something Big privacy policy`,d.append(` covers your player profile, saved progress, browser storage, and optional newsletter signup.`),p(`p`,`privacy disclaimer`,this.el).textContent=`Independent, free-to-play experimental tech demo. Not affiliated with or endorsed by Rockstar Games or Take-Two Interactive. Provided “as is”; bugs, interruptions, and progress resets may occur.`,this.err=p(`div`,`err`,this.el),this.err.setAttribute(`role`,`alert`);let h=p(`div`,`controls`,this.el);h.innerHTML=m(),p(`div`,`ver`,this.el).textContent=`v${n}`,a.addEventListener(`focusin`,()=>r(`keyboard_focus`),{once:!0}),a.addEventListener(`submit`,e=>{e.preventDefault();let n=this.input.value.trim();if(n.length<1){this.err.textContent=`Enter your name (up to 100 characters).`,this.input.focus();return}if(!this.email.validity.valid){this.email.reportValidity();return}this.err.textContent=``,r(`submit`),t(!1),this.onSubmit(n,this.email.value.trim(),this.newsletter.checked)})}setError(e){this.err.textContent=e}clear(){this.input.value=``,this.email.value=``,this.newsletter.checked=!0}show(n=d(u)??``){this.visible||(this.open(),t(!0),r(`form_shown`),this.input.value=n,e()||setTimeout(()=>this.input.focus(),50))}};export{c as i,a as n,i as r,g as t};
//# sourceMappingURL=nameEntry-CwO3iapZ.js.map