/**
 * Named camera spots for screenshots / critic loops. Real coordinates.
 * URL: ?spot=times-square&time=18:00&weather=clear&nohud=1&fov=55
 * heading: compass degrees the camera looks toward. pitch: degrees, negative = down.
 * h: metres above physics.groundHeight (street datum, NOT height above water).
 * Audit placements against footprints/roads/planimetric sidewalks: node tools/check-spots.mjs
 */
import { lonLatToXZ } from '@shared/geo';

export interface Spot { id: string; name: string; lat: number; lon: number; heading: number; pitch: number; h: number; fov?: number }
...
export function spotToWorld(s: Spot): { x: number; y: number; z: number } {
