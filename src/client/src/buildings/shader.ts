/**
 * Facade über-shader chunks, injected into MeshStandardMaterial via onBeforeCompile (material.ts).
 * One shader handles walls (per-style masonry/glass with a procedural window grid, recessed windows with
 * interior mapping, storefronts, dirt), roofs, trims/props, signs and beacons, by the per-vertex `kind`.
 *
 * All patterns are in METRES (uv = metres along / up the wall) so bricks, stones and windows are true scale.
 */
import { HASH_GLSL } from './hash';
import { MASONRY, STYLE_COUNT, SIGN_NAME_ROWS, SIGN_NEON_ROWS } from './styles';
import { FACADE_EMISSIVE_GLSL } from './lighting';
import { WINDOW_GRID_GLSL, WINDOW_LIGHT_GAIN } from './windows';
...
/**
 * Shared by the mid branch (bay-resolution grid, 500 m to the tile edge) and the far skyline layer (far.ts) so the
 * two LODs agree on glass reflectance, prewar shafts and night lighting where they meet.
 */
export const LOD_FACADE_GLSL = /* glsl */ `
...
vec3 brickPattern(vec2 uv, vec3 tint, bool painted, float fade, float fwm, out vec2 tilt, out float rough, out vec3 brickVar) {
  const float BW = ${MASONRY.brick[0]}, BH = ${MASONRY.brick[1]}, MJ = 0.005; // 8 x 2 5/8 in brick, 3/8 in joint (half per side)
...
vec3 stonePattern(vec2 uv, vec3 tint, bool rusticated, float fade, out vec2 tilt, out float rough) {
  float BW = rusticated ? 0.9 : ${MASONRY.stone[0]}, BH = rusticated ? 0.45 : ${MASONRY.stone[1]};
...
vec3 brownstonePattern(vec2 uv, vec3 tint, bool painted, float wear, float fade, out vec2 tilt, out float rough) {
  const float BW = ${MASONRY.stone[0]}, BH = ${MASONRY.stone[1]};
  float row = floor(uv.y / BH);
...
vec3 cmuPattern(vec2 uv, vec3 tint, float fade, out vec2 tilt, out float rough) {
  const float BW = ${MASONRY.cmu[0]}, BH = ${MASONRY.cmu[1]};
  float row = floor(uv.y / BH);
  float bx = uv.x / BW + mod(row, 2.0) * 0.5;
  vec2 f = vec2(fract(bx), fract(uv.y / BH));
...
const float SIGN_NAME_ROWS = ${SIGN_NAME_ROWS}.0;
const float SIGN_NEON_ROWS = ${SIGN_NEON_ROWS}.0;
...
Surf shadeWall(vec3 V, vec3 N, float detail) {
  ...
  if (fw > 0.16) {
    ...
    if (uNight > 0.15) {
      ...
      S.emis += light * lit * win * depth * nightK * uEmissive * ${WINDOW_LIGHT_GAIN.toFixed(2)};
    }
...
/** replaces <normal_fragment_maps>: computes the surface, overrides diffuseColor/roughness/metalness/normal/emissive */
export const FACADE_FRAGMENT_MAIN = /* glsl */ `
{
