...
/**
 * @param {{ admin?: boolean, verbose?: boolean, log?: (msg: string) => void }} [options]
 */
export function createWorld(options = {}) {
  ...
  /**
   * Bind one WebSocket-like object to the world. Called by net.js for every
   * accepted upgrade on `${BASE_PATH}/ws`.
   * @param {{ readyState: number, send: Function, close: Function, terminate?: Function, on: Function }} ws
   */
  function addConnection(ws) {
    ...
    ws.on('close', () => {
      ...
      broadcast({ t: 'online', count: players.size });
      log(`[net] ${player.name} (#${player.id}) left - ${players.size} online`);
    });
  ...
  function start() {
    if (timers.length) return world;
    lastTick = Date.now();
    timers = [
      setInterval(tick, 1000 / C.SERVER_SNAPSHOT_HZ),
  ...
  /**
   * A JSON-safe view of the live world, for the SvelteKit pages and
   * /world/api/status. Not part of the client's wire protocol.
   */
  function status() {
    return {
      ...
      snapshotHz: C.SERVER_SNAPSHOT_HZ,
      aoiRadius: C.AOI_RADIUS_M,
      weather: { ...weather },
  ...
  const world = {
    admin: ADMIN, verbose: VERBOSE,
    addConnection, start, stop, status,
    serverTime, dayFraction,
  };
  return world;
}
