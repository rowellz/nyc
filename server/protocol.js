...
const BinaryKind = { Snapshot: 1, State: 2 };
...
function writePlayerState(view, o, s) {
...
function readPlayerState(view, o, out = emptyState()) {
...
function encodeSnapshot(serverTime, states) {
...
function decodeStateMessage(buf) {
  const v = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
  return { clientTime: v.getFloat64(1, true), state: readPlayerState(v, 9) };
}

module.exports = {
  PROTOCOL_VERSION, GAME_VERSION, BinaryKind, StateFlag, PLAYER_STATE_BYTES,
  emptyState, writePlayerState, readPlayerState, encodeSnapshot, decodeStateMessage,
};
