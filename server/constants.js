...
const ORIGIN = { lat: 40.75362, lon: -73.98322 };
const R_LAT = 110574.0;
const R_LON = 111320.0 * Math.cos((ORIGIN.lat * Math.PI) / 180);

const lonLatToXZ = (lon, lat) => ({ x: (lon - ORIGIN.lon) * R_LON, z: -(lat - ORIGIN.lat) * R_LAT });
const headingToYaw = (deg) => -(deg * Math.PI) / 180;

const TILE_SIZE = 256;
const SAFE_ZONE = { x: 0, z: 0, radius: 115 };
const SPAWN_PROTECTION_SECONDS = 120;
const DAY_LENGTH_SECONDS = 7200;
const CLIENT_STATE_HZ = 20;
const SERVER_SNAPSHOT_HZ = 15;
const AOI_RADIUS_M = 350;
...
const WeaponId = { None: 0, Pistol: 1, SMG: 2, Shotgun: 3, Rifle: 4 };
const WEAPONS = {
...
module.exports = {
  ORIGIN, lonLatToXZ, headingToYaw, TILE_SIZE, SAFE_ZONE, SPAWN_PROTECTION_SECONDS,
  DAY_LENGTH_SECONDS, CLIENT_STATE_HZ, SERVER_SNAPSHOT_HZ, AOI_RADIUS_M, MAX_PLAYERS,
  PLAYER_MAX_HEALTH, PLAYER_HEIGHT, PLAYER_RADIUS, SCORE, SPAWN_POINTS, LANDMARKS,
  WeaponId, WEAPONS,
};
